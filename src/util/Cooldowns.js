...
class Cooldowns {
    ...
    async updateCooldown(cooldownUsage, expires) {
        ...
        if (secondsDiff >= this._dbRequired){
            await cooldownSchema.findOneAndUpdate(
                { _id: key }, { _id: key, expires }, { upsert: true },
            )
        }
    }
    ...
    verifyCooldown(duration) {
        ...
        if ( split.length !== 2 ) {
            throw new Error(`Duration "${duration}" is an invalid duration. Please use "<number> <s/m/h/d>".`)
        }
        ...
        if (quantity <= 0) {
            throw new Error(`Invalid quantity of time "${quantity}". Please use a value greater than 0.`);
        }
